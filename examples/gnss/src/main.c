...
static struct nrf_modem_gnss_pvt_data_frame gnss_data;
...
static void gnss_event_handler(int event)
{
	...
	case NRF_MODEM_GNSS_EVT_PVT:
        ...
        retval = nrf_modem_gnss_read(&gnss_data, sizeof(gnss_data), NRF_MODEM_GNSS_DATA_PVT);
		if (retval) {
			LOG_ERR("nrf_modem_gnss_read failed, err %d", retval);
		}
        if (gnss_data.flags & NRF_MODEM_GNSS_PVT_FLAG_FIX_VALID) {
			LOG_INF("latitude:   %f", gnss_data.latitude);
        	LOG_INF("longitude:  %f", gnss_data.longitude);
		} else {}
		break;
